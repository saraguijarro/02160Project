...
//----2 methods used to find a client in the database by search word, and a potential filter-----------
	...
	public ArrayList<Client> searchClient(String searchword, String filter) {
		...
		if (filter.equals("None")) { //search everything because no filter
			...
			for (int i=0;i<this.clients.size();i++) {
				if (this.clients.get(i).name.contains(searchword)){foundClients.add(this.clients.get(i));}
			}
		}
		else if (filter.equals("Address")) { //search only in the address
			for (int i=0;i<this.clients.size();i++) {
				if (this.clients.get(i).address.getCity().contains(searchword)
				   ||this.clients.get(i).address.getCountry().contains(searchword)
				   ||this.clients.get(i).address.getPostcode().contains(searchword)
				   ||this.clients.get(i).address.getStreetName().contains(searchword)
				   ||this.clients.get(i).address.getStreetNumber().contains(searchword)
				   ||this.clients.get(i).address.getKeyWord().contains(searchword))
					{foundClients.add(this.clients.get(i));}
			}
		}
		else if (filter.equals("Reference Person")) {
			for (int i=0;i<this.clients.size();i++) {
				if (this.clients.get(i).referencePerson.contains(searchword)){foundClients.add(this.clients.get(i));}
			}
		}
		else if (filter.equals("Email")) {
			for (int i=0;i<this.clients.size();i++) {
				if (this.clients.get(i).email.contains(searchword)){foundClients.add(this.clients.get(i));}
			}
		}
